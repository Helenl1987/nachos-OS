...
{ 
	...
	mapentry = new BitmapEntry[numBits];
...
BitMap::Clear(int which) 
{
    ASSERT(which >= 0 && which < numBits);
    map[which / BitsInWord] &= ~(1 << (which % BitsInWord));
	mapentry[which].lru = 0;
	mapentry[which].dirty = FALSE;
	mapentry[which].use = FALSE;
	mapentry[which].readOnly = FALSE;
}
...
int BitMap::Find_Complete(int sectors) {
...
int BitMap::Find_ByTrack(int track) {
	for(int i = 0; i < SectorsPerTrack; i++) {
		if(!Test(track*SectorsPerTrack + i)) {
			Mark(track*SectorsPerTrack + i);
			return track*SectorsPerTrack + i;
		}
	}
	return -1;
}
