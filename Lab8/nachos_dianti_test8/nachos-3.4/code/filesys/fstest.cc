...
void PipeWrite() {
    if (!fileSystem->Create("TestFile1", 50)) {
    	printf("Perf test: can't create %s\n", "TestFile1");
    	return;
    }
	//printf("Perf test: create %s success\n", "TestFile1");
    OpenFile *openFile1;    
    if ((openFile1 = fileSystem->Open("TestFile1")) == NULL) {
		printf("Perf test: unable to open file %s\n", "TestFile1");
		return;
    }
	//printf("Perf test: open1 %s success\n", "TestFile1");
	openFile1->RediWrite("PipeFile");
	char buffer[50];
	printf("please input:\n");
	scanf("%s", buffer);
	openFile1->Write(buffer, strlen(buffer));
}

void PipeRead() {
    if (!fileSystem->Create("TestFile2", 50)) {
    	printf("Perf test: can't create %s\n", "TestFile2");
    	return;
    }
	//printf("Perf test: create %s success\n", "TestFile2");
    OpenFile *openFile2;    
    if ((openFile2 = fileSystem->Open("TestFile2")) == NULL) {
		printf("Perf test: unable to open file %s\n", "TestFile2");
		return;
    }
	//printf("Perf test: open2 %s success\n", "TestFile2");
	openFile2->RediRead("PipeFile");
	char buffer[50];
	openFile2->Read(buffer, 50);
...
PerformanceTest()
{
    ...
    if (!fileSystem->Create("PipeFile", 50)) {
    	printf("Perf test: can't create %s\n", "PipeFile");
    	return;
    }
	printf("Perf test: create %s success\n", "PipeFile");
	...
	printf("File TestFile1:\n");
	fileSystem->PrintFile("TestFile1");
	printf("File TestFile2:\n");
	fileSystem->PrintFile("TestFile1");
	printf("File PipeFile:\n");
	fileSystem->PrintFile("PipeFile");


}

void PipenewthreadTest(int from) {
	printf("thread %d communicate through pipe with thread %d\n", currentThread->getThreadID(), from);
	printf("thread %d read data from pipe\n", currentThread->getThreadID());
	char data[100];
	int length = fileSystem->ReadPipe(data, from, currentThread->getThreadID());
	data[length] = 0;
	printf("pipe output: %s\n", data);
}

void PipeTest() {
	Thread *newthread = new Thread("new pipe thread");
	printf("thread %d communicate through pipe with thread %d\n", currentThread->getThreadID(), newthread->getThreadID());
	char data[100];
	printf("pipe input:");
	scanf("%s", data);
	printf("thread %d write data %s to pipe, length = %d\n", currentThread->getThreadID(), data, strlen(data));
	fileSystem->WritePipe(data, strlen(data), currentThread->getThreadID(), newthread->getThreadID());
	newthread->Fork(PipenewthreadTest, currentThread->getThreadID());
	currentThread->Yield();
}
