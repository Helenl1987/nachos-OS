...
extern bool Send(char *data, int des);
extern char* Receive(int des);
...
//	Thread *t2 = new Thread("forked user thread 2");
	Thread *t3 = new Thread("forked user thread 3");
	t1->Fork(usertest, "../test/matmult");
//	t2->Fork(usertest, "../test/mamama");
	t3->Fork(usertest, "../test/sort");
}
#endif //USER_PROGRAM
...
void MessagenewthreadTest(int from) {
	printf("thread %d communicate through message with thread %d\n",  currentThread->getThreadID(), from);
	printf("thread %d receive message from buffer\n", currentThread->getThreadID());
	char *data = Receive(currentThread->getThreadID());
	ASSERT(data != NULL);
	printf("message output: %s\n", data);
}

void MessageTest() {
	Thread *newthread = new Thread("new message thread");
	printf("thread %d communicate through message with thread %d\n", currentThread->getThreadID(), newthread->getThreadID());
	printf("thread %d send message to buffer\n", currentThread->getThreadID());
	char data[100];
	printf("message input:");
	scanf("%s", data);
	ASSERT(Send(data, newthread->getThreadID()) == TRUE);
	newthread->Fork(MessagenewthreadTest, currentThread->getThreadID());
	currentThread->Yield();
}
