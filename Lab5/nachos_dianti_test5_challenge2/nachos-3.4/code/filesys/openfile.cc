...
{ 
    hdr = new FileHeader;
    hdrSector = sector;
	synchDisk->ReadWriteP(hdrSector);
    hdr->FetchFrom(sector);
	hdr->refercnt = hdr->refercnt + 1;
	//printf("In OpenFile: hdr sector = %d hdr->refercnt = %d\n", sector, hdr->refercnt);
	hdr->WriteBack(sector);
	synchDisk->ReadWriteV(hdrSector);
    seekPosition = 0;
	rediR = NULL;
	rediW = NULL;
}
...
OpenFile::Read(char *into, int numBytes)
{
	...
	int result;
	if(rediR != NULL)
		result = rediR->ReadAt(into, numBytes, 0);
	else
   		result = ReadAt(into, numBytes, seekPosition);
   	seekPosition += result;
   	hdr->lastVisitTime = stats->totalTicks;
   	hdr->WriteBack(hdrSector);
	synchDisk->ReadWriteV(hdrSector);
   	return result;
}
...
{
//	printf("Write: into = %s numBytes = %d seekPosition = %d\n", into, numBytes, seekPosition);
	synchDisk->ReadWriteP(hdrSector);
   	int result;
	if(rediW != NULL)
		result = rediW->WriteAt(into, numBytes, 0);
	else
		result = WriteAt(into, numBytes, seekPosition);
   	seekPosition += result;
   	hdr->lastVisitTime = stats->totalTicks;
   	hdr->lastModifyTime = stats->totalTicks;
   	hdr->WriteBack(hdrSector);
	synchDisk->ReadWriteV(hdrSector);
   	return result;
}
...
void OpenFile::RediRead(char *filename) {
	rediR = fileSystem->Open(filename);
}

void OpenFile::RediWrite(char *filename) {
	rediW = fileSystem->Open(filename);
}
