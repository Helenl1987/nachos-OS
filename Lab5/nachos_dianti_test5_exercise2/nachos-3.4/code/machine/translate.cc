...
// from the virtual address
    ...
    if (tlb == NULL) {		// => page table => vpn is index into table
		//printf("check if page fault\n");
		//printf("vpn = %d tid = %d valid = %d pageTableSize = %d\n", vpn, currentThread->getThreadID(), (int)pageTable[vpn].valid, pageTableSize);
		/*if(pageTable[vpn].valid) {
			printf("mvpn = %d mtid = %d\n",MemoryBitmap->mapentry[pageTable[vpn].physicalPage].vpn,MemoryBitmap->mapentry[pageTable[vpn].physicalPage].tid);
		}*/
		if (vpn >= pageTableSize) {
			...
			for(int j = 0; j < NumPhysPages; j++) {
				if(MemoryBitmap->Test(j) == TRUE && MemoryBitmap->mapentry[j].tid == currentThread->getThreadID() && MemoryBitmap->mapentry[j].vpn == vpn) {
					memoryindex = j;
					break;
				}
			}
			if(memoryindex == -1) {
				pagefault_times++;
				//printf("raise page fault exception\n");
				return PageFaultException;
			}
			else {
				//printf("page found!\n");
				pageFrame = memoryindex;
				bentry = &MemoryBitmap->mapentry[memoryindex];
				MemoryBitmap->mapentry[memoryindex].lru = stats->totalTicks;
			}
		}
		/*else if (pageTable[vpn].valid == FALSE || MemoryBitmap->mapentry[pageTable[vpn].physicalPage].tid != currentThread->getThreadID() || MemoryBitmap->mapentry[pageTable[vpn].physicalPage].vpn != vpn) {
	    	DEBUG('a', "virtual page # %d too large for page table size %d!\n", virtAddr, pageTableSize);
			pagefault_times++;
			//printf("raise page fault exception\n");
	    	return PageFaultException;
		...
		//printf("page found!\n");
		entry = &pageTable[vpn];
		MemoryBitmap->mapentry[pageTable[vpn].physicalPage].lru = stats->totalTicks;*/
    } 
	else {
		/*#ifdef USE_TLB
		//printf("^^^^^^^^^^^^^^^^^^^^^^^^^\n");
        for (entry = NULL, i = 0; i < TLBSize; i++)
    	    if (tlb[i].valid && (tlb[i].virtualPage == vpn)) {
				entry = &tlb[i];			// FOUND!
				tlb[i].lru = stats->totalTicks;
				tlb[i].lfu += 1;
				//printf("hit...i = %d, lfu = %d\n",i,tlb[i].lfu);
				break;
	   		 }
    ...
    if (bentry->readOnly && writing) {	// trying to write to a read-only page
...
//    pageFrame = entry->physicalPage;
    ...
    bentry->use = TRUE;		// set the use, dirty bits
    if (writing) {
//		bentry->dirty = TRUE;
		MemoryBitmap->mapentry[pageFrame].dirty = TRUE;
	}
