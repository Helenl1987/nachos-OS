...
Machine::Machine(bool debug)
{
	...
	vdisk = new char[VDiskSize];
	vdiskentry = new VDiskEntry[NumDiskPages];
	vdiskCurrentNum = 0;
...
{
    if (tlb != NULL) {
		printf("Total memory visit time is %d, TLB miss time is %d, hit rate is %lf\n", totalmemory_times, tlbmiss_times, 1-(double)tlbmiss_times/totalmemory_times);
        delete [] tlb;
	}
	printf("Total memory visit time is %d, Page Fault time is %d, hit rate is %lf\n", totalmemory_times, pagefault_times, 1-(double)pagefault_times/totalmemory_times);
	delete [] mainMemory;
	delete [] vdisk;
	delete [] vdiskentry;
}
