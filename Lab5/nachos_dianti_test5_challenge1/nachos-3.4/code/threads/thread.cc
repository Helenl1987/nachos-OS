...
Thread::Yield ()
{
    ...
    if (nextThread != NULL) {
	// scheduler->ReadyToRun(this);
	//printf("Yielding %s usedTimeSlices=%d nextThread = %s\n", this->getName(), this->getusedTimeSlices(), nextThread->getName());
	//printf("In Thread::Yield, nextThread = %s\n", nextThread->getName());
...
Thread::ShowThreadStatus() {
	printf("Thread name: \"%s\" id = %d ", name, threadID);
	switch(status) {
